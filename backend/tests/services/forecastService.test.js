...
describe('Forecast Service', () => {
  ...
  describe('getClimateZones', () => {
    it('should return a list of climate zones', async () => {
      ...
      expect(climateZones[0]).toHaveProperty('id');
  ...
  describe('saveForecastToHistory', () => {
    it('should save a forecast to user history', async () => {
      ...
      expect(savedForecast.createdAt).toBeDefined();
      expect(savedForecast.userId).toBe(userId);
      expect(savedForecast.params).toEqual(forecastData.params);
      expect(savedForecast.results).toEqual(forecastData.results);
    });
  });
});
